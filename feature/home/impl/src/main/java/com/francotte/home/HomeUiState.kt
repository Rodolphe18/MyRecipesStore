...
sealed interface AmericanRecipes {
    ...
    data class Success(
        val americanRecipes: List<LikeableRecipe>,
    ) : AmericanRecipes
}
...
sealed interface AreasRecipes {
    ...
    data class Success(
        val areasRecipes: Map<String, List<LikeableRecipe>>,
    ) : AreasRecipes
}
...
internal fun Flow<Result<List<LikeableRecipe>>>.mapToAmericanRecipes() =
    map {
        if (it.isSuccess) {
            AmericanRecipes.Success(it.getOrDefault(emptyList()))
        } else {
            AmericanRecipes.Error
        }
    }

internal fun Flow<Result<List<LikeableRecipe>>>.mapToEnglishRecipes() =
    map {
        if (it.isSuccess) {
            EnglishRecipes.Success(it.getOrDefault(emptyList()))
        } else {
            EnglishRecipes.Error
        }
    }

internal fun Flow<Result<Map<String, List<LikeableRecipe>>>>.mapToAreasRecipes() =
    map {
        if (it.isSuccess) {
            AreasRecipes.Success(it.getOrDefault(emptyMap()))
        } else {
            AreasRecipes.Error
        }
    }
