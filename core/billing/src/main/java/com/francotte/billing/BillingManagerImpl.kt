...
class BillingManagerImpl @Inject constructor(
    ...
    override fun startConnection(scope: CoroutineScope) {
        this.scope = scope
    ...
    private suspend fun queryProducts() {
        productQueryLock.withLock {
            ...
            if (result.responseCode == BillingClient.BillingResponseCode.OK) {
                ...
                if (details.isEmpty()) _events.tryEmit("Aucun abonnement trouvé (premium_id).")
            } else {
    ...
    override fun onPurchasesUpdated(billingResult: BillingResult, purchases: List<Purchase?>?) {
        when (billingResult.responseCode) {
            BillingClient.BillingResponseCode.OK -> {
                scope?.launch { handlePurchases(purchases.orEmpty().filterNotNull()) }
            }
    ...
    private suspend fun handlePurchases(purchases: List<Purchase>) {
        val (isPremium, toAcknowledge) = handlePurchaseLock.withLock {
            val premium = purchases.any {
                it.purchaseState == Purchase.PurchaseState.PURCHASED &&
                    it.products.contains(PREMIUM_PRODUCT_ID)
            }
            val tokens = purchases
                .asSequence()
        ...
        userDataRepository.setPremium(isPremium)
