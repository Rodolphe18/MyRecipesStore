...
val LocalInterstitialManager =
    staticCompositionLocalOf<InterstitialManager> {
        error("InterstitialManager not provided")
    }

val LocalBillingController =
    staticCompositionLocalOf<BillingController> {
        error("PremiumController not provided")
    }

val LocalAuthManager =
    staticCompositionLocalOf<AuthManager> {
        error("AuthManager not provided")
    }
val LocalFavoriteManager =
    staticCompositionLocalOf<FavoriteManager> {
        error("FavoriteManager not provided")
    }
val LocalLaunchCounterManager =
    staticCompositionLocalOf<LaunchCounter> {
        error("LaunchCounterManager not provided")
    }
val LocalInAppRatingManager =
    staticCompositionLocalOf<InAppRatingManager> {
