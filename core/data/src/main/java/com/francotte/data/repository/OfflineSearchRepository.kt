...
class OfflineSearchRepositoryImpl(
    private val network: RecipeApi,
) : OfflineSearchRepository {
    override fun getAllIngredients(): Flow<List<NetworkIngredient>> =
        flow {
            emit(network.getAllIngredients().meals)
        }

    override fun getAllAreas(): Flow<List<NetworkArea>> =
        flow {
            emit(network.getAllAreas().meals)
        }

    override fun getAllCategories(): Flow<List<NetworkAbstractCategory>> =
        flow {
