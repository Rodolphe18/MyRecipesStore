...
class Navigator(val state: NavigationState) {
    ...
    fun goBack() {
        when {
            state.rootStack.isNotEmpty() -> {
                if (state.rootStack.last() == state.startKey) error("You cannot go back from the start route")
                state.rootStack.removeLastOrNull()
            }
            state.currentKey == state.startKey -> error("You cannot go back from the start route")
            state.currentKey == state.currentTopLevelKey -> state.topLevelStack.removeLastOrNull()
